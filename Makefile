...
FEDORA_SOURCES := https://src.fedoraproject.org/rpms/libmodulemd1/raw/f$(subst fc,,$(DIST))/f/sources
SRC_FILE := modulemd-$(VERSION).tar.xz

BUILDER_DIR ?= ../..
SRC_DIR ?= qubes-src

DISTFILES_MIRROR ?= https://github.com/fedora-modularity/libmodulemd/releases/download/libmodulemd-$(VERSION)/
